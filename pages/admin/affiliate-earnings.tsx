        ...
        <div className="overflow-auto border border-gray-700 rounded-md">
          <table className="min-w-full text-sm">
            <thead className="bg-[#334155]">
              <tr>
                <th className="p-3 text-left border-r border-gray-600">
                  Affiliate
                </th>
                {months.map((month) => (
                  <th
                    key={month}
                    className="p-3 text-left border-r border-gray-600"
                  >
            ...
            <tbody>
              {rows.map((row, idx) => (
                <tr key={idx} className="border-t border-gray-700">
                  <td className="p-3 border-r border-gray-700 font-semibold">
                    {row.name}
                  </td>
                  {months.map((month) => (
                    <td key={month} className="p-3 border-r border-gray-700">
                      {row.history[month]
                        ? `$${row.history[month].toFixed(2)}`
                        : "-"}
                    </td>
                  ))}
                </tr>
