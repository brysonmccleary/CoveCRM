...
function isOptOut(text: string): boolean {
  ...
  return ["stop","stopall","unsubscribe","cancel","end","quit"].some(k=>t===k) || t.includes("remove") || t.includes("opt out");
}
function isHelp(text: string): boolean { const t=(text||"").trim().toLowerCase(); return t==="help"||t.includes("help"); }
function isStart(text: string): boolean { const t=(text||"").trim().toLowerCase(); return t==="start"||t==="unstop"; }
function containsConfirmation(text: string) {
  const t = (text || "").toLowerCase();
  return [
    "that works","works for me","sounds good","sounds great","perfect",
    "let's do","lets do","confirm","confirmed","book it","schedule it",
    "set it","lock it in","we can do","we could do","3 works","works"
  ].some(p=>t.includes(p));
}
...
function extractRequestedISO(textIn: string, state?: string): string | null {
  ...
  for (const re of patterns) {
    ...
    if (m) {
      ...
      let h=parseInt(m[3],10); const min=m[4]?parseInt(m[4],10):0; const ap=m[5];
      if (ap) { if (ap==="pm"&&h<12) h+=12; if (ap==="am"&&h===12) h=0; }
  ...
  if (bare) {
    let h=parseInt(bare[1],10); const min=bare[2]?parseInt(bare[2],10):0; const ap=bare[3];
    if (ap) { if (ap==="pm"&&h<12) h+=12; if (ap==="am"&&h===12) h=0; }
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (!fromNumber || !toNumber || !body) return res.status(200).json({ message: "Missing required fields, acknowledged." });
    ...
    // persist inbound
    const inboundEntry = { type: "inbound" as const, text: body, date: new Date() };
    ...
    if (usConversation && !approved) {
      lead.interactionHistory.push({ type: "inbound", text: "[note] Auto-reply suppressed: A2P not approved yet.", date: new Date() });
      await lead.save();
      if (io) io.to(user.email).emit("message:new", { leadId: lead._id, type: "inbound", text: "[note] Auto-reply suppressed: A2P not approved yet.", date: new Date() });
      return res.status(200).json({ message: "A2P not approved; no auto-reply sent." });
    }
    if ((lead as any).unsubscribed) return res.status(200).json({ message: "Lead unsubscribed; no auto-reply." });

    // If this reply is from a retention campaign, don't engage AI
    const assignedDrips = lead.assignedDrips || [];
    const isClientRetention = assignedDrips.some((id) => id.includes("client_retention"));
    ...
    if (!requestedISO && containsConfirmation(body)) {
      requestedISO = extractTimeFromLastAI(lead.interactionHistory || [], stateCanon) ||
    ...
    if (!requestedISO) {
      try {
        ...
        aiReply = lastAI?.text?.trim() === v ? `Shoot me a time that works (e.g., “tomorrow 3:00 pm”) and I’ll text a confirmation.` : v;
      }
    }
    ...
    setTimeout(async () => {
      try {
        ...
        const params: Parameters<Twilio["messages"]["create"]>[0] = { ...baseParams, body: aiReply };
...
async function getSendParams(userId: string, toNumber: string, fromNumber: string) {
  if (process.env.TWILIO_MESSAGING_SERVICE_SID) {
    return { messagingServiceSid: process.env.TWILIO_MESSAGING_SERVICE_SID, to: fromNumber } as Parameters<Twilio["messages"]["create"]>[0];
  }
  const a2p = await A2PProfile.findOne({ userId });
  if (a2p?.messagingServiceSid) {
    return { messagingServiceSid: a2p.messagingServiceSid, to: fromNumber } as Parameters<Twilio["messages"]["create"]>[0];
  }
  // NOTE: Scheduling is NOT supported when sending with a bare "from" number.
  return { from: toNumber, to: fromNumber } as Parameters<Twilio["messages"]["create"]>[0];
}
