...
function bufferToStream(buffer: Buffer): Readable {
  const s = new Readable(); s.push(buffer); s.push(null); return s;
}
function last10(v?: string | null) { if (!v) return; const d = String(v).replace(/\D+/g, ""); const k = d.slice(-10); return k || undefined; }
function lcEmail(v?: string | null) { if (!v) return; const s = String(v).trim().toLowerCase(); return s || undefined; }
function lc(v?: string | null) { return v ? String(v).trim().toLowerCase() : undefined; }
...
function normalizeState(v?: string | null) { if (!v) return; const k = String(v).trim().toUpperCase(); return STATE_MAP[k] || STATE_MAP[k.replace(/\s+/g,"_")] || undefined; }
function sanitizeStatus(v?: string | null) { if (!v) return; const s = String(v).trim(); if (!s) return; return s.toLowerCase().split(/\s+/).map(w=>w[0]?.toUpperCase()+w.slice(1)).join(" "); }

/* NEW-FOLDER-ONLY helpers */
...
async function selectImportFolder(userEmail: string, opts: { folderName?: string; sourceHint?: string }) {
...
/* JSON body */
type JsonPayload = { folderName?: string; newFolderName?: string; newFolder?: string; name?: string; mapping?: Record<string,string>; rows?: Record<string,any>[]; skipExisting?: boolean; };
async function readJsonBody(req: NextApiRequest): Promise<JsonPayload | null> {
  ...
  const chunks: Buffer[] = [];
  await new Promise<void>((resolve,reject)=>{ req.on("data",(c)=>chunks.push(Buffer.from(c))); req.on("end",()=>resolve()); req.on("error",(e)=>reject(e)); });
...
function mapRow(row: Record<string, any>, mapping: Record<string, string>) {
  const pick = (k: string) => { const col = mapping[k]; if (!col) return undefined; const v = row[col]; return typeof v === "string" ? v.trim() : v; };
  const first = pick("firstName"); const last = pick("lastName"); const email = pick("email"); const phone = pick("phone");
  const stateRaw = pick("state"); const notes = pick("notes"); const source = pick("source");
  const leadTypeRaw = row["Lead Type"] || row["leadType"] || row["LeadType"];
  const statusRaw = pick("status") ?? pick("disposition");
  const mergedNotes = source && notes ? `${notes} | Source: ${source}` : source && !notes ? `Source: ${source}` : notes;
  const normalizedState = normalizeState(stateRaw); const emailLc = lcEmail(email); const phoneKey = last10(phone);
  const status = sanitizeStatus(statusRaw) || "New";
  return { "First Name": first, "Last Name": last, Email: emailLc, email: emailLc, Phone: phone, phoneLast10: phoneKey, normalizedPhone: phoneKey, State: normalizedState, Notes: mergedNotes, leadType: sanitizeLeadType(leadTypeRaw || ""), status };
}
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (json) {
    try {
      ...
      if (!mapping || !rows || !Array.isArray(rows)) return res.status(400).json({ message: "Missing mapping or rows[]" });

      const provided = (json.newFolderName || json.newFolder || json.name || json.folderName || "").toString().trim();
      const { folder, safeName } = await selectImportFolder(userEmail, { folderName: provided });
      ...
      const existing = ors.length ? await Lead.find({ userEmail, $or: ors }).select("_id phoneLast10 normalizedPhone Email email folderId") : [];
      const byPhone = new Map<string, any>(); const byEmail = new Map<string, any>();
      for (const l of existing) {
        const p1 = l.phoneLast10 && String(l.phoneLast10); const p2 = l.normalizedPhone && String(l.normalizedPhone);
        const e1 = l.Email && String(l.Email).toLowerCase(); const e2 = l.email && String(l.email).toLowerCase();
        if (p1) byPhone.set(p1, l); if (p2) byPhone.set(p2, l); if (e1) byEmail.set(e1, l); if (e2) byEmail.set(e2, l);
      }

      const ops: any[] = []; const processedFilters: any[] = []; let skipped = 0;
      for (const m of mapped) {
        ...
        const filter = buildFilter(userEmail, phoneKey, emailKey); if (!filter) { skipped++; continue; }

        const base: any = { ownerEmail: userEmail, folderId: folder._id, folder_name: String(folder.name), "Folder Name": String(folder.name), updatedAt: new Date() };
        ...
        if (exists) {
          ...
          const setOnInsert: any = { userEmail, status: m.status || "New", createdAt: new Date() };
      ...
      if (ops.length) {
        ...
        const existedOps = processedFilters.length - inserted; updated = existedOps < 0 ? 0 : existedOps;
        ...
        if (processedFilters.length) {
          ...
          if (ids.length) { await Folder.updateOne({ _id: folder._id, userEmail }, { $addToSet: { leadIds: { $each: ids } } }); }
        }
      }

      return res.status(200).json({ message: "Import completed", folderId: folder._id, folderName: folder.name, counts: { inserted, updated, skipped }, mode: "json-new-folder-only", safeName, skipExisting });
    } catch (e: any) {
  ...
  form.parse(req, async (err, fields, files) => {
    if (err) { console.error("❌ Form parse error:", err); return res.status(500).json({ message: "Form parse error" }); }

    try {
      const providedNameRaw = (fields as any).newFolderName ?? (fields as any).newFolder ?? (fields as any).name ?? fields.folderName;
      const providedName = (Array.isArray(providedNameRaw) ? providedNameRaw[0] : providedNameRaw)?.toString()?.trim() || "";
      ...
      const skipExisting = (Array.isArray(fields.skipExisting) ? fields.skipExisting[0] : fields.skipExisting)?.toString() === "true";

      const sourceHint = file.originalFilename || providedName;
      const { folder, safeName } = await selectImportFolder(userEmail, { folderName: providedName, sourceHint });
      ...
      if (rawRows.length === 0) return res.status(400).json({ message: "No data rows found in CSV (empty file or header-only)." });
      ...
      if (!mappingStr) {
        ...
        return res.status(200).json({ message: "Leads imported successfully", count: leadsToInsert.length, folderId: folder._id, folderName: folder.name, safeName, mode: "multipart-legacy-new-folder-only" });
      }
      ...
      return res.status(200).json({ message: "Leads imported successfully", folderId: folder._id, folderName: folder.name, counts: { inserted, updated, skipped }, mode: "multipart+mapping-new-folder-only", safeName, skipExisting });
    } catch (e: any) {
