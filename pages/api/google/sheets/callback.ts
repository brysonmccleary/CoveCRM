...
export default async function handler(
  ...
  // Canonical Sheets redirect URI – must match Google Console + env
  const redirectUri =
    process.env.GOOGLE_REDIRECT_URI ||
    `${baseUrl(req).replace(/\/$/, "")}/api/google/sheets/callback`;
  ...
  await User.updateOne(
    { email },
    {
      $set: {
        ...
        // ✅ Keep calendar in sync so calendar code can reuse token
