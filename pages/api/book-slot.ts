...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const end = new Date(start.getTime() + user.bookingSettings.slotLength * 60000);
  ...
  try {
    const event = await calendar.events.insert({
      ...
      requestBody: {
        ...
        start: { dateTime: start.toISOString(), timeZone: user.bookingSettings.timezone },
        end: { dateTime: end.toISOString(), timeZone: user.bookingSettings.timezone },
    ...
    res.status(200).json({ message: "Booking confirmed", eventId: event.data.id });
  } catch (err) {
