...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (!phoneNumber || !phoneNumber.startsWith("+")) {
      return res.status(400).json({ message: "Provide phoneNumber in E.164 format, e.g. +14155551234" });
    }
    ...
    if (!msSid) {
      return res.status(400).json({ message: "No Messaging Service for this user. Run /api/a2p/start first." });
    }
    ...
    if (!owned || owned.length === 0) {
      return res.status(404).json({ message: "This Twilio account does not own the provided phone number." });
    }
    ...
    // Update/Upsert PhoneNumber collection
    ...
    return res.status(500).json({ message: err?.message || "Failed to assign number" });
  }
}
