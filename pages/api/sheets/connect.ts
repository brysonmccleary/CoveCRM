...
function _hmacHexFromString(bodyString, secret) {
  const bodyBytes = Utilities.newBlob(String(bodyString || ""), "application/json").getBytes();
  const keyBytes = Utilities.newBlob(String(secret || ""), "text/plain").getBytes();
  const rawSig = Utilities.computeHmacSha256Signature(bodyBytes, keyBytes);
  return rawSig
...
function covecrmBackfillWorker() {
  ...
  try {
    ...
    try {
      ...
      Logger.log("❌ Backfill error: " + String(err));
    } catch {}
  } finally {
...
function _postBackfillBatch(runId, rows, totalRows) {
  ...
  if (code < 200 || code >= 300) {
    let snippet = "";
    try {
      const txt = resp.getContentText ? String(resp.getContentText() || "") : "";
      snippet = txt ? txt.slice(0, 300) : "";
    } catch {}
    throw new Error("Backfill batch failed with " + code + (snippet ? (" | " + snippet) : ""));
  }
}
