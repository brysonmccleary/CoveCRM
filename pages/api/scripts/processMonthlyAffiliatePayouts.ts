...
// API route (token-protected) so it can be run by a scheduler
export default async function handler(
  ...
  // Auth via bearer header or ?token=
  ...
  try {
      ...
      try {
        ...
        try {
          await sendAffiliatePayoutEmail({
            ...
            dashboardUrl: `${baseUrl}/affiliates/earnings`,
          });
        } catch {
          // non-fatal
        }
    ...
    console.error("processMonthlyAffiliatePayouts error:", err);
