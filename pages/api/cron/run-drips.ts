...
function sortDaySteps(drip: any): Array<{ text: string; day: string }> {
  const steps: Array<{ text: string; day: string }> = Array.isArray(drip?.steps) ? drip.steps : [];
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!shouldRunNowPT()) {
    return res.status(200).json({ message: "Not run window (expects 9:00 AM PT). Set DRIPS_DEBUG_ALWAYS_RUN=1 to override." });
  }
