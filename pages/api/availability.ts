...
export default async function handler(
  ...
  if (
    ...
    return res
      .status(404)
      .json({ message: "User not found or not properly connected" });
  }

  const { accessToken, refreshToken, expiryDate } = user.googleSheets;

  const { workingHours, slotLength, bufferTime, timezone, maxPerDay } =
    user.bookingSettings;
  ...
  try {
    ...
    while (current < endOfDay) {
      ...
      current = new Date(
        current.getTime() + (slotLength + bufferTime) * 60 * 1000,
      );
    }
