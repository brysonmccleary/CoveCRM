...
export default function DayViewPage() {
  ...
  const { startISO, endISO } = useMemo(() => {
    const safe = (date || "").match(/^\d{4}-\d{2}-\d{2}$/)
      ? date!
    ...
    <div className="min-h-screen bg-[#0b0f19] text-white p-6">
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          ...
          <Link href="/calendar">
            <span className="px-4 py-2 rounded bg-slate-700 hover:bg-slate-600 cursor-pointer">
              Back to Calendar
            </span>
          </Link>
        </div>
        ...
        {loading ? (
          ...
          <ul className="space-y-3">
            {events.map((e) => (
              <li
                key={e.id}
                className="bg-[#121826] rounded p-4 border border-slate-700"
              >
                ...
                {e.location && (
                  <div className="text-sm mt-1 opacity-80">📍 {e.location}</div>
                )}
                {e.description && (
                  <div className="text-sm mt-2 opacity-90">{e.description}</div>
                )}
              </li>
            ))}
          </ul>
