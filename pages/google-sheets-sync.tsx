...
/** Minimal, session-only support for *adding* a custom field from the selector */
const ADD_CUSTOM_TOKEN = "__ADD_CUSTOM__";
function sanitizeFieldKey(input: string): string {
  // Mongo-safe: no dots; no leading $
  let key = (input || "").trim();
  key = key.replace(/\./g, "_");
...
export default function GoogleSheetsSyncPage() {
  ...
  // NEW: per-row custom-add state (UI only)
  ...
  // combined options for the select
  const combinedOptions: FieldOption[] = [...defaultFields, ...customFields];
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          if (v === ADD_CUSTOM_TOKEN) {
                            setCustomMode((cm) => ({ ...cm, [h]: true }));
                            setCustomInput((ci) => ({ ...ci, [h]: "" }));
                            return;
                          }
                          setMapping((m) => ({ ...m, [h]: v }));
                        }}
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          onClick={() => {
                            ...
                            // add if missing
                            if (
                              !defaultFields.some((f) => f.value === sanitized) &&
                              !customFields.some((f) => f.value === sanitized)
                            ) {
                              setCustomFields((cf) => [
                                ...cf,
                            ...
                            setMapping((m) => ({ ...m, [h]: sanitized }));
                            setCustomMode((cm) => ({ ...cm, [h]: false }));
                            setCustomInput((ci) => ({ ...ci, [h]: "" }));
                          }}
