...
export function buildCreativeGenerationSignature(draft: Record<string, any>): string {
  ...
  const canonical = {
    ...
    layoutId: clean(draft?.layoutId),
    executionId: clean(draft?.cssExecutionId),
    compositionVariant: clean(draft?.cssCompositionVariant),
    macroFamily: clean(draft?.cssMacroFamily),
    hierarchyTreatment: clean(draft?.cssHierarchyTreatment),
    panelStructure: clean(draft?.cssPanelStructure),
    typographyTreatment: clean(draft?.cssTypographyTreatment),
    selectorPresentation: clean(draft?.cssSelectorPresentation),
    benefitTreatment: clean(draft?.cssBenefitTreatment),
    backgroundTreatment: clean(draft?.cssBackgroundTreatment),
    ctaTreatment: clean(draft?.cssCtaTreatment),
