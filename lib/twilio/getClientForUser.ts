 ...
 *
 * We create the TwiML App INSIDE the subaccount (AP SIDs are account-scoped).
 * Store it on the user so /api/twilio/voice/token can always grant outgoingApplicationSid.
 */

async function ensureSubaccountTwimlAppForUser(args: {
  email: string;
  baseUrl: string;
  subSid: string;
  apiKeySid: string;
  apiKeySecret: string;
}): Promise<{ twimlAppSid: string }> {
  const normalizedEmail = (args.email || "").toLowerCase().trim();
  if (!normalizedEmail) throw new Error("ensureSubaccountTwimlAppForUser: missing email");

  const subSid = sanitizeId(args.subSid || "");
  const apiKeySid = sanitizeId(args.apiKeySid || "");
  const apiKeySecret = String(args.apiKeySecret || "").trim();

  if (!subSid.startsWith("AC")) throw new Error("ensureSubaccountTwimlAppForUser: invalid subaccount SID");
  if (!apiKeySid.startsWith("SK") || !apiKeySecret) throw new Error("ensureSubaccountTwimlAppForUser: invalid subaccount API key");

  // Load writable user doc (we need to persist twimlAppSid)
  const fresh = await User.findOne({ email: normalizedEmail });
  if (!fresh) throw new Error(`ensureSubaccountTwimlAppForUser: user not found: ${normalizedEmail}`);

  const existing =
    (fresh as any)?.twimlAppSid ||
  ...
  const sid = String(app?.sid || "").trim();
  if (!sid.startsWith("AP")) {
    throw new Error("ensureSubaccountTwimlAppForUser: TwiML app create did not return AP sid");
  }

  // Store in both places to cover any legacy reads
